...
export default function Navbar(props: NavbarProps) {
      ...
      <>
        {links.map(([link, name]) => (
          <li className="flex justify-center" key={link}>
            <a
              className={`px-3 py-1 rounded-xl text-center text-(--navbar-link) transition duration-300 ${whichLink === link ? "!text-white font-semibold bg-[#383838]" : "hover:text-(--navbar-link-hover) "}`}
              href={link}
              onClick={() => {
                setWhichLink(link);
              }}
  ...
  return (
    <header className="z-50 w-full fixed top-0 p-4 self-center flex justify-center backdrop-blur-xs backdrop-darken bg-black/50 shadow-[0_5px_15px_rgba(000,000,000,0.5)]">
